...
import kotlinx.android.synthetic.main.bottomsheetlayoutm.*
...
class Post : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        progress_small155555.setVisibility(View.GONE)
    ...
    private fun uploadToFirebase(uri: Uri) {
        ...
        fileRef.putFile(uri)
                    ...
                    fileRef.downloadUrl.addOnSuccessListener(object : OnSuccessListener<Uri?> {
                        override fun onSuccess(p0: Uri?) {
                            if (radioBtn == btnRadio5){
                                ...
                                hashMap.put("key",key.key.toString())
                                hashMap.put("uid", FirebaseAuth.getInstance().currentUser?.uid.toString())
                                hashMap.put("title",aaa5.text.toString())
                                hashMap.put("des",aaa55.text.toString())
                                hashMap.put("link",p0.toString())
                                hashMap.put("tag","11th")
                                hashMap.put("ads",false)
                                key.setValue(hashMap)
                                val hashMap1 = HashMap<String,Any>()
                                hashMap1.put("User",FirebaseAuth.getInstance().currentUser?.uid.toString())
                                FirebaseDatabase.getInstance().getReference("Notes")
                                    .child(key.key.toString()).child("views")
                                    .child(FirebaseAuth.getInstance().currentUser?.uid.toString()).updateChildren(hashMap1)
                                FirebaseDatabase.getInstance().getReference("Notes")
                                    .child(key.key.toString()).child("report")
                                    .child(FirebaseAuth.getInstance().currentUser?.uid.toString()).updateChildren(hashMap1)
                            ...
                            progress_small155555!!.visibility = View.GONE
                            Toast.makeText(
                                this@Post,
            ...
            }).addOnProgressListener { progress_small155555!!.visibility = View.VISIBLE }
